...
BASE_URL = os.getenv("LOCAL_DIFY_BASE_URL", "http://127.0.0.1:18080").removesuffix("/v1")
...
def _auth_headers(env_name: str) -> dict:
    """Build Dify auth headers from env; skip instead of hardcoding app keys."""
    api_key = os.getenv(env_name)
    if not api_key or api_key == "__SET_LOCALLY__":
...
def test_student_guidance_chatflow():
    ...
    headers = _auth_headers("LOCAL_DIFY_WORKFLOW_GUIDANCE_KEY")
    body = {
        "inputs": {},
...
def test_teacher_diagnosis_workflow():
    ...
    headers = _auth_headers("LOCAL_DIFY_WORKFLOW_SUMMARY_KEY")
    body = {
        "inputs": {
...
def test_ai_grading_profile_workflow():
    ...
    url = f"{BASE_URL}/v1/workflows/run"
    headers = _auth_headers("LOCAL_DIFY_WORKFLOW_GRADING_KEY")
    body = {
        "inputs": {
            "mode": "grading",
