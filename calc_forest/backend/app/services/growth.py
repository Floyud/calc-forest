...
def _load_json_cached(filename: str) -> list[dict]:
    return _load_json(filename)


def list_tree_species() -> list[TreeSpecies]:
    return [TreeSpecies(**_map_species(item)) for item in _load_json_cached("tree_species.json")]
...
def list_encouragement_rules() -> list[EncouragementRule]:
    return [EncouragementRule(**item) for item in _load_json_cached("encouragements.json")]
