...
from app.schemas import ERROR_LABELS, Student, StudentProfile
...
async def get_weak_knowledge_points(student_id: str) -> list[dict]:
    ...
    async with get_db() as db:
        try:
            ...
            logger.warning("Failed to query weak knowledge points for %s: %s", student_id, e)
            return []
...
async def get_class_weak_points(class_id: str) -> list[dict]:
    ...
    async with get_db() as db:
        try:
            ...
            logger.warning("Failed to query class weak points for %s: %s", class_id, e)
            return []
...
async def get_student_profile(student_id: str) -> StudentProfile | None:
    ...
    # Calculate per-error-code "safety" accuracy:
    # For each error code E, accuracy = 1 - (count of E errors / total answers).
    # This represents how well the student avoids this specific error type.
    # A high value means the student rarely makes this mistake.
    ...
    if rows:
        ...
        for code, count in error_counter.items():
            accuracy_by_error_code[code] = round(1.0 - count / total, 4)
    ...
    error_counter = Counter(
        r["error_code"] for r in rows if r["error_code"] and r["error_code"] != "OK"
    )
    ...
    for r in dh_rows:
        ...
        try:
            ...
            week_num = 0
        if week_num > 0:
            weekly_group.setdefault(week_num, []).append(bool(r["is_correct"]))
    ...
    try:
        ...
        logger.warning("Failed to get growth milestone for %s: %s", student_id, e)
    ...
    try:
        ...
        for r in reversed(rows):
            if r["is_correct"]:
                if current_correct_streak == 0 and current_wrong_streak > 0:
                    break
                current_correct_streak += 1
            else:
                if current_wrong_streak == 0 and current_correct_streak > 0:
                    break
                current_wrong_streak += 1

        mastery_count = sum(
            1 for acc in accuracy_by_error_code.values() if acc >= 0.85
        )

        archetype_result = classify_student(
            accuracy=accuracy,
            accuracy_trend=recent_accuracy_trend,
            total_attempts=total_attempts,
            current_streak_correct=current_correct_streak,
            current_streak_wrong=current_wrong_streak,
            mastery_count=mastery_count,
        )
        profile.archetype = {
            "name": archetype_result.archetype,
            "id": archetype_result.archetype_id,
            "description": archetype_result.description,
            "confidence": archetype_result.confidence,
            "evidence": archetype_result.evidence,
        }
    except Exception as e:
        logger.warning("Failed to classify archetype for %s: %s", student_id, e)
