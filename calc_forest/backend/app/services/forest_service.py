...
async def get_class_forest(class_id: str) -> ClassForestResponse | None:
    async with get_db() as db:
        ...
        for row in await stats_cursor.fetchall():
            r = dict(row)
            stats_by_student[r["student_id"]] = r
        ...
        for sid in student_ids:
            stats = stats_by_student.get(sid)
            total_attempts = stats["total"] if stats else 0
            correct_count = stats["correct"] if stats else 0
            overall_accuracy = round(correct_count / total_attempts, 4) if total_attempts > 0 else 0.0

            error_counter: Counter = Counter()
            if stats and stats.get("error_breakdown"):
                for part in stats["error_breakdown"].split(","):
                    pieces = part.split(":")
                    if len(pieces) == 3:
                        ...
                        if ec and ec != "OK" and err_count > 0:
                            ...
                            all_class_errors.extend([ec] * err_count)
