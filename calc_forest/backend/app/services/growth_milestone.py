...
def compute_stage(
    days_completed: int | float,
    accuracy: float | None = None,
    mastery_count: int = 0,
) -> str:
    """Map days_completed to growth stage, adjusted by learning quality."""
    base_stage = "seed"
    for i in range(len(STAGE_THRESHOLDS) - 1, -1, -1):
        if days_completed >= STAGE_THRESHOLDS[i]:
            base_stage = STAGE_KEYS[i]
            break

    if accuracy is None or base_stage not in STAGE_KEYS:
        return base_stage

    stage_idx = STAGE_KEYS.index(base_stage)

    if accuracy >= 0.80 and mastery_count >= 3 and stage_idx < len(STAGE_KEYS) - 1:
        return STAGE_KEYS[stage_idx + 1]

    if accuracy < 0.50 and days_completed > 7 and stage_idx > 0:
        return STAGE_KEYS[stage_idx - 1]
...
async def record_practice_day(student_id: str) -> dict:
    ...
    if cycle is None:
        raise NotFoundException(f"年级 {student.grade} 当前没有学习周期")
