...
_cycle_cache: dict[int, AcademicCycle | None] = {}
...
async def get_current_cycle(grade: int) -> AcademicCycle | None:
    global _cycle_cache, _cache_date
    today = date.today()
    if _cache_date == today and grade in _cycle_cache:
        return _cycle_cache[grade]
    today_iso = today.isoformat()
    async with get_db() as db:
        cursor = await db.execute(
            "SELECT * FROM academic_cycles "
            "WHERE grade = ? AND start_date <= ? AND end_date >= ?",
            (grade, today_iso, today_iso),
        )
        row = await cursor.fetchone()
    if _cache_date != today:
        _cycle_cache = {}
        _cache_date = today
    result = _row_to_cycle(row) if row else None
    _cycle_cache[grade] = result
    return result
