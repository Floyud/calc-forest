...
_SCHEMA_SQL = """
-- NOTE: FK + CASCADE constraints are defined here for fresh DB creation.
-- Existing DBs will NOT be affected (SQLite ALTER TABLE cannot add FK to
-- existing columns). To apply FKs to an existing DB, delete calc_forest.db
-- and re-seed. PRAGMA foreign_keys=ON is set in init_db().
...
CREATE TABLE IF NOT EXISTS student_answers (
    ...
    UNIQUE(homework_id, student_id, problem_sequence),
...
CREATE TABLE IF NOT EXISTS exercise_types (
    ...
    -- Practice problems within session
    CREATE TABLE IF NOT EXISTS student_practice_problems (
        id TEXT PRIMARY KEY,
        ...
        error_code TEXT,
        answered_at TEXT,
        FOREIGN KEY (session_id) REFERENCES student_practice_sessions(id) ON DELETE CASCADE
    );

    -- Per-student quiz answers
    CREATE TABLE IF NOT EXISTS quiz_student_answers (
        ...
        UNIQUE(quiz_id, student_id, problem_sequence),
        FOREIGN KEY (quiz_id) REFERENCES quiz_sessions(id) ON DELETE CASCADE,
        FOREIGN KEY (student_id) REFERENCES students(id) ON DELETE CASCADE
    );
...
_MIGRATE_SQL = [
    ...
    # Add created_at to student_answers for trend queries
    "ALTER TABLE student_answers ADD COLUMN created_at TEXT NOT NULL DEFAULT ''",
    "CREATE INDEX IF NOT EXISTS idx_diagnosis_history_error_code ON diagnosis_history(error_code)",
    "CREATE INDEX IF NOT EXISTS idx_diagnosis_history_student_id ON diagnosis_history(student_id)",
    "CREATE INDEX IF NOT EXISTS idx_student_answers_error_code ON student_answers(error_code)",
    ...
    "CREATE INDEX IF NOT EXISTS idx_homework_pdfs_class_id ON homework_pdfs(class_id)",
...
VALUES ('T001', '王老师', '13800000001', '6d40ab0e2ffb225ca0bd46e5b48f71d70d74d13f3037cffea45756d9e6acbf37', '["G6C1"]');
"""
    ...
    for sql in _MIGRATE_SQL:
        try:
            if sql.strip().startswith("ALTER TABLE"):
                ...
                tbl = parts[2]
                col = parts[5]
                cur = await _db.execute(f"PRAGMA table_info({tbl})")
