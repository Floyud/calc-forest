...
class HomeworkGenerateRequest(BaseModel):
    ...
    difficulty: str = Field(default="A")
...
class SimulatedRecognitionAnswer(BaseModel):
    problem_sequence: int
    raw_answer: str
    notes: str | None = None
...
class OCRUploadRequest(BaseModel):
    homework_id: str
    student_id: str
    answers: list[SimulatedRecognitionAnswer] = Field(default_factory=list)
    source_label: str = "blank_area_simulation"
...
class OCRTaskResponse(BaseModel):
    ...
    grading_status: HomeworkStatus = HomeworkStatus.SUBMITTED
    archive_status: ArchiveStatus = ArchiveStatus.PENDING
    review_status: str = "pending_teacher_review"
    uploaded_at: str
    reviewed_at: str | None = None
    recognized_answers: list[RecognizedAnswer] = Field(default_factory=list)
    submission_id: str | None = None
    diagnosis: HomeworkGradeResult | None = None


class HealthResponse(BaseModel):
    ok: bool = True
    service: str = "primary-math-diagnosis-agent"
    version: str = "0.1.0"
...
class QuizGenerateRequest(BaseModel):
    ...
    error_codes_target: list[str] = Field(default_factory=list)
    problem_count: int = Field(default=5, ge=1, le=10)
    difficulty: str = Field(default="B")
