...
def test_teacher_summary_node_fallback():
...
def test_teacher_summary_node_success():
    node = TeacherSummaryNode()
    diagnosis = _make_diagnosis()
    context = {"diagnosis": diagnosis, "student_id": "S001", "grade": 6}
...
def test_teacher_summary_node_should_run_with_diagnosis():
    node = TeacherSummaryNode()
    import asyncio
    assert asyncio.get_event_loop().run_until_complete(
        node.should_run({"diagnosis": _make_diagnosis()})
    ) is True


def test_teacher_summary_node_should_run_without_diagnosis():
    node = TeacherSummaryNode()
    import asyncio
    assert asyncio.get_event_loop().run_until_complete(
        node.should_run({})
